		...
		function (taskJson, encoding, done) {
			...
			validateTask(folderName, task)
				.then(function () {
					...
					var dependenciesjson = path.join(tgtPath, 'dependencies.json');
					if (fs.existsSync(dependenciesjson)) {
						...
						if (dependencies.archivePackages) {
							...
							archives.forEach(function (archive) {
								gutil.log('Download archive dependencies from:' + archive.url);
								gutil.log('Unzip to: ' + archive.dest);
								download(archive.url)
									.pipe(unzip())
									.pipe(gulp.dest(archive.dest));
							});
						}

						shell.rm(dependenciesjson);
						shell.popd();
					}
